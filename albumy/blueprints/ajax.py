...
def collect(photo_id):
    ...
    if current_user.is_collecting(photo):
        return jsonify(message='Already collect'), 403
    current_user.collect(photo)
    # 如果当前用户不是图片作者并且图片作者能够接受收藏消息提醒，就给图片的作者推送收藏消息提醒
...
def follow(username):
    if not current_user.is_authenticated:
        return jsonify(message='Login require'), 403
    if not current_user.confirmed:
        return jsonify(message='Conform account require'), 400
